...
const handleCancel = () => {
  ...
  if(modelVisible.value.payment_status === Enum['payment_status']['PAID'].value) {
    ...
    >
      <v-card-text>
        <div style="display: flex; flex-direction: column;justify-content: center; align-items: center;">
          <template v-if="modelVisible.payment === Enum['payment']['CASH'].value">
            <v-icon color="success" size="100">mdi-check-circle</v-icon>
            <div class="text-h5 mt-3">
              {{ info.name }} đã thanh toán thành công
            </div>
          </template>
          <template v-else-if="modelVisible.payment === Enum['payment']['TRANSFER'].value">
            <template v-if="modelVisible.payment_status === Enum['payment_status']['PAID'].value">
              <v-icon color="success" size="100">mdi-check-circle</v-icon>
              <div class="text-h5 mt-3">
                {{ info.name }} đã thanh toán thành công
              </div>
            </template>
            <template v-else-if="modelVisible.payment_status === Enum['payment_status']['PENDING'].value">
              <v-img
                :src="qrCodeUrl"
                width="200"
                height="200"
      ...
      <v-card-actions>
        <div class="w-100 d-flex justify-center align-center mb-3">
          <v-btn color="success" border="success" @click="handleCancel" variant="outlined" width="100">OK</v-btn>
        </div>
      </v-card-actions>
    </v-card>
  </v-dialog>
